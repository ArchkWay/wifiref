    ...
    public void subscribe(){
        ...
        Disposable dispTopic = wsClient.mStompClient.topic("/user/queue/measurement/delete")
                ...
                .subscribe(topicMessage -> {
                    ...
                    AllInterface.iLog.addToLog(new LogItem("Подписка удалить измерение","Принято "+topicMessage.getPayload(),null));
                    FRefreshMeasurement fRefreshMeasurement = mGson.fromJson(topicMessage.getPayload(), FRefreshMeasurement.class);

                }, throwable -> {
                    AllInterface.iLog.addToLog(new LogItem("Подписка удалить измерение","Ошибка подписки",null));
    ...
    public void send() {
        ...
        wsClient.compositeDisposable.add(wsClient.mStompClient.send(stompMessage)
                ...
                .subscribe(() -> {
                    ...
                    AllInterface.iLog.addToLog(new LogItem("Отправка удалить измерение","Данные отпрвлены удачно",null));
                }, throwable -> {
                    Log.d(TAG, "Error send STOMP echo", throwable);
                    AllInterface.iLog.addToLog(new LogItem("Отправка удалить измерение","Ошибка отправки",null));
