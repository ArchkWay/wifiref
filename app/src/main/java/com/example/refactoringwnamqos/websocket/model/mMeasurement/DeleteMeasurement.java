...
public class DeleteMeasurement {
    ...
    public DeleteMeasurement(WSClient wsClient){
        this.wsClient = wsClient;
    }

    public void subscribe(){
        // Receive greetings
        Disposable dispTopic = wsClient.mStompClient.topic("/user/queue/measurement/delete")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(topicMessage -> {
                    Log.d(TAG, "Received " + topicMessage.getPayload());
                    Date date = new Date();
                    AllInterface.iLog.addToLog(new LogItem("Подписка удалить измерение","Принято "+topicMessage.getPayload(), String.valueOf(date)));
                    FRefreshMeasurement fRefreshMeasurement = mGson.fromJson(topicMessage.getPayload(), FRefreshMeasurement.class);

                }, throwable -> {
                    Date date = new Date();
                    AllInterface.iLog.addToLog(new LogItem("Подписка удалить измерение","Ошибка подписки", String.valueOf(date)));
    ...
    public void send() {
        ...
        Log.d(TAG, "gson measurement/delete "+gsonStr);

        List<StompHeader> headers = new ArrayList<>();
        String sid = String.valueOf(System.currentTimeMillis());
        headers.add(new StompHeader("sid", sid));
        headers.add(new StompHeader(StompHeader.DESTINATION, "/app/measurement/delete"));
        ...
        wsClient.compositeDisposable.add(wsClient.mStompClient.send(stompMessage)
                ...
                .subscribe(() -> {
                    ...
                    Date date = new Date();
                    AllInterface.iLog.addToLog(new LogItem("Отправка удалить измерение","Данные отпрвлены удачно", String.valueOf(date)));
                }, throwable -> {
                    Log.d(TAG, "Error send STOMP echo", throwable);
                    Date date = new Date();
                    AllInterface.iLog.addToLog(new LogItem("Отправка удалить измерение","Ошибка отправки", String.valueOf(date)));
